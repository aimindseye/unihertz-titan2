...
if [[ -n "${GRADLE_BIN:-}" ]]; then
    ...
    GRADLE="$(find "$GRADLE_USER_HOME/wrapper/dists/gradle-8.13-bin" \
        -maxdepth 6 -type f -path '*/gradle-8.13/bin/gradle' \
        -perm -111 -print -quit 2>/dev/null || true)"
fi

if [[ -z "${GRADLE:-}" || ! -x "$GRADLE" ]]; then
    echo "ERROR: Gradle 8.13 executable not found." >&2
    echo "Set GRADLE_BIN=/path/to/gradle or populate the canonical Sable Gradle cache." >&2
    exit 3
fi
