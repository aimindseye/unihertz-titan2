...
interest=re.compile(r'key|input|touch|gesture|swipe|motion|event|keyboard|pointer|trackpad',re.I)
        ...
        try:
            ...
            for _ in range(dm+vm):
                ...
                while j < len(units):
                    ...
                    j += 1
                if has404 and midx < len(methods):
                    cls,name=methods[midx]
                    text=' '.join([cls,name]+[a+' '+b for a,b in refs])
                    if interest.search(text):
                        reftext='; '.join(a+'->'+b for a,b in refs if interest.search(a+' '+b))[:20]
