...
| Tier 2 E. Restore and deployment contract | **PARTIAL — E0/E1/E2 PASS; E3 pending** | Restore set verified and hashed; bootloader-fastboot preflight passed; fastbootd preflight passed with product `Titan_2`, `is-userspace=yes`, slot `a`, unlocked bootloader, snapshot state `none`, and 9 GiB super. Still required: exact Sable artifact preflight, reviewed AVB/LP/userdata policy, then the first bounded Sable deployment. |
