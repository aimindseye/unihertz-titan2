...
| Tier 2 L. Power / thermal / suspend | **PARTIAL — deep idle + thermal/Health healthy; charging policy closure remains** | Natural deep-idle entry is accepted from the >20-minute unplugged session plus DeviceIdle history; the post-idle dump showed charging only because USB was reconnected for ADB. Thermal HAL and Health/battery evidence are healthy. Below-full charging progression, power-button wake, alternate charging and battery-health policy remain to close; bounded throttling is intentionally deferred. |
